...
public class ParquetFormat implements Format<Hdfs3SinkConnectorConfig, Path>
{
    private final AvroData avroData;
    
    public ParquetFormat(final HdfsStorage storage) {
    ...
    public RecordWriterProvider<Hdfs3SinkConnectorConfig> getRecordWriterProvider() {
        return (RecordWriterProvider<Hdfs3SinkConnectorConfig>)new ParquetRecordWriterProvider(this.avroData);
    }
    
    @Override
    public SchemaFileReader<Hdfs3SinkConnectorConfig, Path> getSchemaFileReader() {
        return (SchemaFileReader<Hdfs3SinkConnectorConfig, Path>)new ParquetFileReader(this.avroData);
    }
    
    @Override
    public HiveFactory getHiveFactory() {
        return (HiveFactory)new ParquetHiveFactory();
    }
}
